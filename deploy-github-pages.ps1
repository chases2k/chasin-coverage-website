# Deploy Chasin Coverage website (folder "chasin-coverage-website 2") to free GitHub Pages
...
if ($LASTEXITCODE -ne 0) {
  Write-Host "Logging in (browser will open)..."
  & $gh auth login --hostname github.com --git-protocol https --web
  if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
}

$user = (& $gh api user --jq .login).Trim()
if (-not $user) { Write-Host "Could not read GitHub username"; exit 1 }
...
if ($pending) {
  git commit -m "Deploy Chasin Coverage site (website 2) to GitHub Pages"
} else {
...
Write-Host " Wait ~1–3 min (videos make first deploy a bit slower)."
